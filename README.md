...
- 2. DB생성 스크립트 실행
- 3. DB매니저실행하기
- 현재 까지 작업한 소스를 여러분 이름 도메인으로배포예정 hyowon-spring5.herokuapp.com
...
#### 20210625(금) 작업.
- Pull이 안되는 원인: 로컬 이클립스에서 commit할 것이 남아있으면 PULL 않됨. 해결책은: 이클립스에서 커밋 후 다시 PULL로 해결.
- 게시물관리 Create 작업 마무리.
...
- GET: Insert (외부사이트의 입력폼에서도 입력 가능) - search 할 때, 쿼리스트링으로 데이터 전송. url?key=value&key2=value2(주소에 입력됨.)
- POST: Insert (외부사이트에서 입력 불가능, 같은 사이트의 입력 폼에서 가능) - CRUD 할 때, form의 입력태그(히든스트링)로 데이터 전송.
...
####20210609(수) 작업내역.
- 프로젝트를 진행: 보유기술 70% + 신기술 30%(개발자도 모름) = 9할성공
- PageVO.java 클래스 생성 마무리.
- JUnit에서 위 작업한 내용을 기준으로 selectMember() 테스트 진행.
- <![CDATA[ 쿼리내용 ]]> 태그 안쪽의 기호(부등호같은것)를 쓸 수 있게 문자열을 변환하는 수식.
- 쿼리에서 변수와 문자열을 연결 할 때는 +(자바)X, ','X, ||으로 사용한다.
- JUnit에서 회원관리 나머지 CRUD 테스트 진행.
- 컨트롤러를 이용하여 관리자단 회원관리화면 JSP 만들기 진행예정.

####20210608(화) 작업내역.
- 페이징에 사용되는 변수(쿼리변수+VO변수 포함)
- queryStartNo, queryPerPageNum, page, perPageNum, startPage, endPage
- 검색에 사용되는 변수(쿼리변수만): 검색어변수(search_keyword), 검색조건변수(search_type)

```
--SQL쿼리로 페이징을 먼저 구현해서 변수로 삼을것을 정의한다.
--PageVO의 멤버변수로 사용 예정.
SELECT TB.* FROM
    (
    ...
    ) TA WHERE ROWNUM <= (0*5)+5 --보여줄 레코드의 수 예: (page*b)+b (0번째 페이지*5개출력)+5개출력 = 5개 출력
) TB WHERE TB.RNUM > 0*5 --위에서 보여줄 수에서 범위 지정 예: page*b 0번째페이지*5개출력 = 총 0개 제외. 총 5개 중 맨 앞번호부터 0개를 제외한다.
--페이징쿼리에서 필요한 변수는 2개 필요함.
--현재페이지 개수의 변수: page*b == queryStartNo
--1페이지당보여줄 개수의 변수: b == queryPerPageNum
--PageVO에서 필요한 추가변수(아래)
--UI하단의 페이지 선택개수의 변수: c == perPageNum
--perPageNum 변수를 받아서 startPage, endPage를 구해서 하단 페이지 선택 번호를 출력.
```
...
- IF인터페이스 만드는 목적: 복잡한 구현클래스를 간단하게 구조화 시켜서 개발자가 관리하기 편하게 정리하는 역할 -> 정처기 캡슐화 구현과 관련있음.
- 스프링부트에서는 4번 서비스 클래스가 없고 바로 컨트롤러로 이동합니다.
- 4. Service(서비스)클래스생성(서비스에 쿼리결과를 담아 놓습니다.)(1개)
- Junit에서 위 작업한 내용을 CRUD테스트 해본다. -> 대리 혹은 사원에게 아래 작업을 맡김.
...
- 마이바티스 추가 순서(아래)
- 1. pom.xml에 모드 추가
- 2. 마이바티스 설정파일 추가(쿼리를 저장할 위치,파일명 지정)
- JUnit로 CRUD 실습(아래). - 코딩시작
...
- 우리가 하는 책 스프링 웹프로젝트는 개발 전자정부표준프레임워크 개발 툴. => 기반은 이클립스 기반을 확장. => 전자정부표준프레임워크 툴 커스터마이징(제외)
--------------------------------------------------
- 스프링에서는 마이바티스를 이용해서 쿼리를 관리하게 됩니다.(세팅해야함)
- 예전에는 자바코드(jsp코드)안에 쿼리를 만들어서 프로그램을 만들었음. -> 문제점 발견(스파게티코딩)
- MVC로 분리: Model부분의 SQL쿼리를 분리하도록 기능을 추가한 것이 마이바티스 입니다.
- 마이바티스(mybatis)의 형은 아이바티스(ibatis)로, 현재는 거의 쓰지 않음. -> 마이바티스(현재)
- 마이바티스가 나오기 전에는 쿼리를 모아서 프로시저로 만들어서 작업했음.
- 오라클의 DB관리 로그인정보(Application Express웹프로그램 이름): admin/apmsetup -> 암호변경요청: Apmsetup1234%(대문자+특수문자+숫자)

```
--SQL디벨로퍼에서 system으로 로그인 후 아래 쿼리로 XE2사용을 완벽하게 지우기(아래)
SELECT * FROM all_users;
--all_users는 테이블X, 시노님(동의어)
--오라클은 스프링과 같은 방식으로 패키지명 안에 함수 혹은 프로시저를 만들 수 있다.
--패키지명이 길어서 개발자가 사용하기가 부담된다.
--위 상황을 해결하도록 만들어진것이 시노님(동의어).
DROP USER XE2 CASCADE;
--XE사용자가 생성한 테이블까지 모두 지우는 명령.
--CSS(Cascade계층형 Style Sheet)
```

- 현업에서는 오픈소스인 mysql(마리아DB)를 사용할 기회가 더 많음.
- 오라클은 납품시 SW비용이 산정이 되어서 사용할 기회가 적지만 장점은 개발자가 상대적으로 적기때문에 고급기술로 볼 수 있다.
- 보통은 학교 전산실 혹은 시청 전산실, 기업 전산실에 XE사용자를 추가해달라고 요청해서 발급받은 계정으로 개발을 시작함.
- 오라클 기초이론 마무리: 시퀀스(스프링에서 사용-AutoIncreament자동증가기능)
- 시퀀스(AI)기능: 게시물을 작성할때 첫번째,두번째 게시물 등등을 구별하는 숫자를 계속 추가하려면
- 현재까지 저장된 게시물의 최고 번호(Max)값을 구해서 +1해야하는데 개발자가 Insert시 매번 해주기가 번거로워서
- DB에서 기본으로 위 기능을 수행하는 로직을 만들었다. 이 기능이 시퀀스(AI).
- 우리 스프링 프로젝트에서는 두개의 시퀀스를 만듭니다.(게시물 시퀀스SEQ_BNO,댓글 시퀀스SEQ_RNO)
- 시노님: 긴객체를 개발자가 타이핑하기 번거로워서 만든 단축이름. 예)sys.dual, -> dual 단축이름으로 사용가능
- 오라클: 더미데이터 일괄등록 예정. 회원관리(100명), 게시판관리(공지사항50개,겔러리50개)
- 위 더미데이터는 프로시저(함수)라는 오라클 DB프로그램 방식으로 추가합니다.
- 오라클: 댓글은 수동등록 후 마무리.

####20210604(금) 작업내역.
- 오라클일때: localhost:1521/XE 접속 URL끝의 XE = 서비스(서버)ID명(현재 두개의 XE,XE2는 스키마명입니다.)
- 오라클은 사용자명(XE)이 스키마명(DB명)입니다. 사용자(XE2) 워크스페이스를 추가하면 테이블스페이스(DB)가 새로 만들어집니다.(구버전은 데스크탑프로그램 -> 현재는 웹프로그램으로 사용자 추가/DB(테이블스페이스)추가 합니다.)
- Mysql(마리아DB): localhost:3306/XE URL끝의 XE = 스키마명(DB명)
- junit테스트시 SQL에러를 디버그하는 방법은 jdbc드라이버 -> log4jdbc드라이버 바꾸면, SQL에러가 나오게됩니다.
- junit에서는 SQL에러가 보입니다. 콘솔창에는 보이지 않습니다. 콘솔창에서 SQL로그 상황이 나오는 드라이버를 pom.xml에 추가하면 됨.
- 오라클: 07장 마무리 후 13장으로 바로 실습예정.(나머지는 아래 실습 후)
- 스프링: JUnit테스트: oldQueryTest메서드 실습 예정.
- 오라클: 08,09,10,11,12,14장 실습예정.

####20210603(목) 작업내역.
- 작업비중(시간) = 관리자단프로그램(70%), 사용자단프로그램(30%)
- 스프링에서 오라클 연동 순서 2가지:
- 1. jdbc(Java DataBase Connection) 확장모듈 pom에 추가
- 2. 오라클 접속 드라이버 확장 모듈을 pom에 추가하지 않고 직접 jar파일을 추가.
- root-context.xml 파일에 오라클 커넥션 빈(스프링클래스)을 추가.
- 스프링관리하는 클래스를 추가하는 방법 2-1: @Controller, @Repository, @Service, @Componant
- 스프링관리하는 클래스를 추가하는 방법 2-2: -context.xml에서 빈(bean)을 추가하면 스프링 클래스가 된다.
- JUnit테스트: 오라클 연동 한 후 회원관리부분 CRUD 테스트 진행 예정.
- 오라클 04장부터 CRUD 실습예정.
- admin 회원관리(jsp디자인)부터 프로그램 작업 시작예정.

####20210602(수) 작업내역
- 쓰레드(Thread): 실행되는 프로그램을 명시, 1개 프로그램에는 보통 1개의 쓰레드가 실행, 특이한 경우(http웹네트워크 프로그램인 경우-게시물 다운로드 버튼 다운로드 100G 쓰레드가 실행되면서, 리스트 버튼을 클릭했을 때 목록보기 쓰레드가 동시에 실행. 안드로이드앱에서 필요.) 여러개 쓰레드 실행이 필요하기도 합니다.
- 예외처리: Throwable 오브젝트 실행시 에러가 발생하면 예외내용을 보낼 때(던질때) 사용하는 클래스.(throws Exception)
- 스프링앱에서는 예외(에러)정보를 대부분 스프링이 throws Exception으로 넘겨서(예외의 전파) 처리합니다.(100개중 2~3개만 개발자가 처리)
- 자바앱에서는 예외(에러)정보를 개발자가 처리합니다.(개발자 손이 많이 갑니다.) 지금 실습.
- 예외처리 하는 목적: 에러발생시 프로그램이 중지되는 것을 방지하고, 계속 프로그램을 사용할 수 있도록 처리하는 것이 주목적.(Exception)/ 에러상황을 세련되게 넘기도록 처리.
...
- admin 폴더 만든이후 분해 OK, Junit 실습 후 작업합니다.
- Junit(Java Unit Test): 자바 단위 테스트. 본 프로그램 작업 전에 프로토타입을 만드는 것. - JunitCRUD 작업 후 시작.
- 로거의 레벨: DEBUG > INFO > WARN > ERROR > FATAL
- 개발할때 : DEBUG 으로 로거 레벨 설정
- 납품할때 : WARN 으로 로거 레벨 변경
- admin 부터 프로그램 작업 시작예정.
- 4장 패키지와 예외처리 실습 후 오라클 CRUD 실습.

####20210601(화) 작업내역
- 에러: 404(File not found 잘못된경로), 500(자바 프로그램 에러)
- 프로젝트의 버전을 올립니다. -> 외부라이브러리 부분의 버전을 올림 - 메이븐 컴파일러 -> pom.xml에서 버전 관리 함.(수정하면 됨) -> 메이븐 업데이트 꼭 해야함.
- Controller클래스에서 생성한 변수 사용(자바) model객체전송(스프링) return home -> home.jsp 출력
- 출력하는 방식1: 25년전부터 있던 방식. EL(Express Language)방식출력-${변수}, <%= ${} %>
- 많이 사용하는 방식: JSTL방식(*표준) - 출력, 반복, 조건 등등 다양한 방식
