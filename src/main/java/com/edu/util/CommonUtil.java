...
public class CommonUtil {
   ...
   @ResponseBody
   public String file_delete(@RequestParam("save_file_name")String save_file_name) { //Ajax는 결과를 '직접'받아야해서 예외처리를 스프링에 보내(throws Exception)지 않고, try~catch문으로 처리한다.
	   String result = ""; //Ajax로 보내는 변수 값
	   try {
   ...
   //다운로드 처리도 같은 페이지에서 결과값만 반환받는 @ResponseBody사용
   ...
   public FileSystemResource download(@RequestParam("save_file_name") String save_file_name, @RequestParam("real_file_name")String real_file_name, HttpServletResponse response) throws Exception {
	   ...
	   response.setContentType("application/download; utf-8"); //아래한글, PPT문서 등에서 한글깨짐을 방지하기 위한 코드 추가
	   real_file_name = URLEncoder.encode(real_file_name); //IE에서 URL이 한글 일 때 에러발생 방지코드 추가
